...
> use os equivalentes: `ls X` → `Test-Path X` · `mkdir -p X` → `New-Item -ItemType Directory -Force X` ·
> `grep -q "s" X && echo OK` → `if (Select-String "s" X -Quiet) { "OK" }`.
> Os comandos `python -c ...` e `pytest ...` são iguais nos dois sistemas.
...
- [Knowledge Base](.github/context/_index.yaml)
- [PROMPT Template](docs/dev/templates/PROMPT_TEMPLATE.md)
